...
# Default-Stop:      0 1 6
# Short-Description: Start and stop the redis database server snmp agent
# Description:       Controls the launch/stop of redis SNMP agent.
### END INIT INFO

PATH=/sbin:/bin:/usr/sbin:/usr/bin
DAEMON=/usr/sbin/redis-snmp
DAEMON2="/usr/bin/perl"
NAME=redis-snmp
DESC="Redis-Server SNMP Agent"
PID_FILE=/var/run/redis-snmp.pid

test -x $DAEMON || exit 0

. /lib/lsb/init-functions

# Include redis-snmp defaults if available
if [ -f /etc/default/redis-snmp ] ; then
	. /etc/default/redis-snmp
fi
...
case "$1" in
  ...
  status)
    ...
    status=$?
    if [ $status -eq 0 ]; then
            log_success_msg "redis-snmp server is running."
    else
            log_failure_msg "redis-snmp server is not running."
    fi
    exit $status
    ;;
  *)
